...
namespace MineDemo.World
{
    public class WorldManager : MonoBehaviour
    {
        ...
        public Dictionary<Vector3Int, BlockType> globalModifications = new Dictionary<Vector3Int, BlockType>();
        ...
        void UpdateChunks()
        {
            ...
            foreach (var pos in chunksToUnload)
            {
                ...
                {
                    foreach (var blockPos in chunkProceduralBlocks[pos].Keys)
                    {
                        // Remove from the world if it hasn't been explicitly overwritten by player
                        // (We just remove the procedural entry. GetExpectedBlock will fall back to terrain)
        ...
        public BlockType GetExpectedBlock(int worldX, int worldY, int worldZ)
        {
            ...
            // Tìm trong Procedural Blocks
            // (Duyệt qua các chunk đang load xem có chunk nào sinh block này không)
            foreach (var chunkBlocks in chunkProceduralBlocks.Values)
            {
        ...
        public void SetGlobalBlock(int worldX, int worldY, int worldZ, BlockType type, byte waterLevel = 0, bool isWaterUpdate = false)
        {
            ...
            {
                globalWaterLevels.Remove(pos); // Xoá level nếu block không còn là nước
            }
        ...
        public void SetProceduralBlock(int worldX, int worldY, int worldZ, BlockType type, int ownerChunkX, int ownerChunkZ)
        {
            ...
            {
                chunkProceduralBlocks[ownerChunk] = new Dictionary<Vector3Int, BlockType>();
            }

            chunkProceduralBlocks[ownerChunk][pos] = type;
